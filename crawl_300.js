...
export async function crawl300Unique() {
  console.log('🚀 Bắt đầu cào 300 truyện BL không trùng nhau từ dualeotruyen...');
  ...
  // Thu thập thêm danh sách truyện từ các danh mục
  const categories = [
    '/the-loai/boylove',
    '/the-loai/dam-my',
    '/the-loai/yaoi',
  ...
  for (let i = 0; i < selectedBooks.length; i++) {
    ...
    try {
      ...
      fullBooks.push({
        ...
        chapters: [],
        totalChapters: 0,
        isHot: hotSlugSet.has(item.slug),
        hotRank: hotMap.get(item.slug) ?? null,
      });
    }
...
if (process.argv[1]?.endsWith('crawl_300.js')) {
  crawl300Unique().catch(console.error);
}
