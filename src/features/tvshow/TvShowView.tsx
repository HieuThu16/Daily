...
export function TvShowView() {
  ...
  const handleSetStatus = async (videoId: string, status: VideoStatus, e?: React.MouseEvent) => {
    ...
    await updateVideoStatusRecord(videoId, 'tvshow', status, {
  ...
  const handleCycleStatus = async (videoId: string, e?: React.MouseEvent) => {
    ...
    const targetVideo = allVideos.find((v) => v.video_id === videoId)
    await cycleNextVideoStatus(videoId, 'tvshow', currentSt, {
      title: targetVideo?.title,
      channel_name: targetVideo?.creator_name || undefined,
      series_key: targetVideo?.series_key,
    })
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const statusSets = getVideoStatusSets('tvshow', supabaseWatchedIds)
  ...
  const categoryGroups = useMemo(() => {
    return groupVideosByCategory(allVideos, 'tvshow', watchedSet, inProgressSet, categoryOverrides)
  ...
  const channelsWithCounts = useMemo(() => {
    ...
    const stats = new Map<string, { total: number; inProgress: number; watched: number }>()
    for (const v of allVideos) {
      const key = v.creator_id || v.creator_name || 'manual'
      const stat = stats.get(key) ?? { total: 0, inProgress: 0, watched: 0 }
      stat.total += 1
    ...
    return channels.map((c) => {
      ...
      return stat ? { ...c, videoCount: stat.total, inProgressCount: stat.inProgress, watchedCount: stat.watched } : c
    })
  }, [channels, allVideos, watchedSet, inProgressSet, videoCounts])
      ...
      {selectedVideoForMenu && (
        ...
        >
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            >
              <Tag size={16} /> Sửa thể loại video
            </button>
...
function ChannelDetailView({
  ...
  const refreshStatuses = async () => {
    ...
    const sets = getVideoStatusSets('tvshow', supabaseWatchedIds)
  ...
  useEffect(() => {
    if (isPlayerActive && playingId) {
      ...
      void autoMarkVideoWatching(playingId, 'tvshow', watched, {
        title: v?.title,
  ...
  return (
    <div className="tv-detail">
      {/* 1. Header Top Bar (Quay lại < | Tên Kênh | Bookmark) */}
      <div className="tv-detail-bar">
        ...
        <div className="tv-detail-title-wrap">
          <Tv size={18} className="tv-header-icon" />
      ...
      {loading ? (
        ...
        <>
          {/* 2. KHUNG TRÌNH PHÁT VIDEO CHÍNH (Chiếm nửa trên) */}
...
function AddTvShowMovieModal({ onClose, onSaved, initialUrl = '' }: { onClose: () => void; onSaved: () => void; initialUrl?: string }) {
  ...
  const handleSaveSingle = async () => {
    ...
    const seriesErr = await ensureManualSeries(seriesKey, title.trim(), creatorId, cleanChannel, 1, now)
    if (seriesErr) {
      setBusy(false)
