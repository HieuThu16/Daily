  ...
  useEffect(() => {
    ;(async () => {
      ...
      const [h, tl, wl, t, td, e, m, tm, wm, brl, o, p, we, nt, nw, wt, sl, sw] = await Promise.all([
        ...
        supabase.from('media_items').select('*').eq('log_date', today).eq('status', 'COMPLETED').is('deleted_at', null),
        supabase.from('media_items').select('*').gte('log_date', week[0].key).lte('log_date', week[6].key).eq('status', 'COMPLETED').is('deleted_at', null),
