...
function parsePrice(value: string) {
  ...
  if (normalized.endsWith('k')) return Math.round((Number.parseFloat(normalized) || 0) * 1000)
    ...
    <section className="page-shell is-narrow" style={{ display: 'grid', gap: 10 }}>
      ...
      {loading ? <p className="muted" style={{ textAlign: 'center', padding: 24 }}>Đang tải…</p> : (
        <>
          ...
          {tab === 'sleep' && periodMode !== 'day' && <SleepPeriodView logs={periodSleepLogs} days={periodRange.days} onDelete={deleteSleep} />}
        </>
      )}
