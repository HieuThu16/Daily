-- Nhập 80 kỷ niệm từ Memory Tree, gắn vào người "vợ".
-- Chạy Ở PROJECT ĐÍCH (ejcwwiohwgidksablzjl) trong SQL Editor,
-- SAU khi đã chạy migration 20260830000000_shared_events_person.sql.

-- 1) Tạo người "vợ" (đánh dấu người yêu chung) nếu chưa có.
insert into public.people (user_id, name, group_key, is_partner)
select u.id, 'vợ', 'FRIEND', true
from auth.users u
where u.email = 'truongnguyenminhhieu100@gmail.com'
  and not exists (
...
where name = 'vợ' and deleted_at is null
  and user_id = (select id from auth.users where email = 'truongnguyenminhhieu100@gmail.com');

-- 2) Nhập 80 kỷ niệm, owner = Hiếu, person_id = người "vợ".
insert into public.shared_events (owner_id, person_id, title, note, event_date, event_time, location, image_url)
select u.id, w.id, v.title, v.note, v.event_date, v.event_time, v.location, v.image_url
from (select id from auth.users where email = 'truongnguyenminhhieu100@gmail.com') u,
     (select id from public.people
        where name = 'vợ' and is_partner and deleted_at is null
          and user_id = (select id from auth.users where email = 'truongnguyenminhhieu100@gmail.com')
        limit 1) w,
(values
...
-- ── Chia sẻ 2 chiều giữa hai tài khoản ─────────────────────────────────
-- Mỗi dòng (user_id, partner_email) = "user_id cho partner_email xem sự kiện của mình".
-- Cả hai account phải đã đăng nhập app ít nhất 1 lần (để có trong auth.users).
...
) as p(owner_email, email) on lower(u.email) = lower(p.owner_email)
